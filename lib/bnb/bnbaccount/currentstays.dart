...
class CurrentStays extends StatefulWidget {
  const CurrentStays({super.key});

  @override
  State<CurrentStays> createState() => _CurrentStaysState();
}

class _CurrentStaysState extends State<CurrentStays> {
  List<BookingModel> _CurrentStayss = [];
  ...
  Future<void> _initializeCustomer() async {
    ...
    await _loadCurrentStayss(resetPage: true);
  }

  Future<void> _loadCurrentStayss({bool resetPage = false}) async {
  ...
  Future<void> _loadMoreBookings() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        setState(() {
          _CurrentStayss.addAll(bookings);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: SingleMGAppBar(
        local.currentStaysTitle,
        context: context,
        isTitleCentered: true,
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : _CurrentStayss.isEmpty
          ? Center(
              child: Column(
          ...
          : RefreshIndicator(
              onRefresh: () => _loadCurrentStayss(resetPage: true),
              child: ListView.builder(
                controller: _scrollController,
                padding: const EdgeInsets.all(16),
                itemCount: _CurrentStayss.length + (_isLoadingMore ? 1 : 0),
                itemBuilder: (context, index) {
                  if (index == _CurrentStayss.length) {
                    return const Center(
                      child: Padding(
                  ...
                  final booking = _CurrentStayss[index];
